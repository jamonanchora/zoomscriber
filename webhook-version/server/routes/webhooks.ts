...
zoomWebhookRouter.post("/", async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Bot installed event - capture account_id and robot_jid (for reference, not needed for webhooks)
    ...
    if (event === "bot_installed" || event === "bot.installed" || event?.toLowerCase() === "bot_installed") {
      ...
      if (accountId) {
        // Save account_id and robot_jid to database (for reference)
        ...
        console.warn("bot_installed webhook missing accountId (not critical for webhook-based version)");
      }
      console.log("=====================================");
    ...
    if (event === "team_chat.channel_reaction_added" || event === "team_chat.dm_reaction_added") {
      ...
      try {
        ...
        console.log("✓ Transcription completed and sent via webhook");
      } catch (err) {
